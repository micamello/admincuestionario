            ...
            <?php              
              if (!empty(METODO_CUESTIONARIO)) {
                ...
                foreach (METODO_CUESTIONARIO as $key => $v) {
                  $ruta = PUERTO.'://'.HOST.'/filtrarEntrevistados2/M'.$key.'/1/'.$key.'/';
            ...
            <?php
              ...
              if (!empty($residenciaActual)) {
                ...
                foreach ($residenciaActual as $key => $v) {
                  $ruta = PUERTO.'://'.HOST.'/filtrarEntrevistados2/R'.$key.'/1/'.$_SESSION['filtrar_consultados']['M'].'/';
            ...
            <?php       
              ...
              if (!empty($empresas)) {
                ...
                foreach ($empresas as $key => $v) {
                  $ruta = PUERTO.'://'.HOST.'/filtrarEntrevistados2/I'.$key.'/1/'.$_SESSION['filtrar_consultados']['M'].'/';
        ...
        <?php
          if (!empty($escolaridad)) {
            ...
            foreach ($escolaridad as $key => $v) {
              $ruta = PUERTO.'://'.HOST.'/filtrarEntrevistados2/E'.$key.'/1/'.$_SESSION['filtrar_consultados']['M'].'/';
             ...
             if (!empty($competencias)) {
                ...
                foreach ($competencias as $key => $v) {
                  ...
                  foreach ($v['grados'] as $p => $val) {
                    ...
                    $ruta2 = PUERTO.'://'.HOST.'/filtrarEntrevistados2/H'.$key.'_'.$p.'/1/'.$_SESSION['filtrar_consultados']['M'].'/';
          ...
          <div class="filtros">
            <?php
              ...
              foreach (EDAD as $key => $v) {
                $ruta = PUERTO.'://'.HOST.'/filtrarEntrevistados2/F'.$key.'/1/'.$_SESSION['filtrar_consultados']['M'].'/';
      ...
      <div class="panel panel-default shadow-panel1">
        ...
        <?php
           ...
           foreach ($nacionalidad as $key => $v) {
                $ruta = PUERTO.'://'.HOST.'/filtrarEntrevistados2/N'.$key.'/1/'.$_SESSION['filtrar_consultados']['M'].'/';
                if($key == SUCURSAL_PAISID){

                  echo '<li><input type="checkbox" name="list" id="pais_'.$key.'">
                      <label style="content:none; cursor:pointer" for="pais_'.$key.'"><a href="'.$ruta.'">'. utf8_encode(ucfirst(strtolower($v['nombre']))).'</a></label>
                      <ul id="menu11" class="interior">';
                  foreach ($v['provincias'] as $p => $val) {

                    $ruta2 = PUERTO.'://'.HOST.'/filtrarEntrevistados2/N'.$key.'_'.$p.'/1/'.$_SESSION['filtrar_consultados']['M'].'/';
                    echo '<li><a href="'.$ruta2.'">'.utf8_encode(ucfirst(strtolower($val))).'</a></li>';
                  }
                  echo '</ul></li>';
                }else{
        ...
        <div class="panel-body">
          ...
          <?php
            ...
            foreach (GENERO as $key => $v) {
              $ruta = PUERTO.'://'.HOST.'/filtrarEntrevistados2/G'.VALOR_GENERO[$key].'/1/'.$_SESSION['filtrar_consultados']['M'].'/';
          ...
          <div class="filtros">
            <?php
              ...
              foreach (ESTADO_CIVIL as $key => $v) {
                $ruta = PUERTO.'://'.HOST.'/filtrarEntrevistados2/C'.$key.'/1/'.$_SESSION['filtrar_consultados']['M'].'/';
            ...
            <?php
              if (!empty($profesion)) {
                ...
                foreach ($profesion as $key => $v) {
                  $ruta = PUERTO.'://'.HOST.'/filtrarEntrevistados2/P'.$key.'/1/'.$_SESSION['filtrar_consultados']['M'].'/';
            ...
            <?php
              if (!empty($ocupacion)) {
                ...
                foreach ($ocupacion as $key => $v) {
                  $ruta = PUERTO.'://'.HOST.'/filtrarEntrevistados2/O'.$key.'/1/'.$_SESSION['filtrar_consultados']['M'].'/';
