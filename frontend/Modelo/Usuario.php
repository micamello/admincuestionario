...
WHERE
  ...
  public static function obtieneNombres($idusuario){
    ...
    $sql = "SELECT nombres, apellidos,id_usuario,grafico FROM mfo_usuariom2 WHERE id_usuario = ? LIMIT 1";
    $rs = $GLOBALS['db']->auto_array($sql,array($idusuario));
    if (empty($rs)){
      $rs = $GLOBALS['db2']->auto_array($sql,array($idusuario));
    }
    return $rs;
  }
  ...
  public static function editarGrafico($uri,$id_usuario){

    if (empty($uri) || empty($id_usuario)) {return false;}

    $rs = $GLOBALS['db']->update('mfo_usuariom2',array('grafico'=>$uri),'id_usuario='.$id_usuario);
    if (empty($rs)){
      $rs = $GLOBALS['db2']->update('mfo_usuariom2',array('grafico'=>$uri),'id_usuario='.$id_usuario);
    }
