...
if (isset($_SERVER['HTTP_HOST']) /*&& (strpos($_SERVER['HTTP_HOST'], 'www.') !== false)*/){
  if (file_exists(FRONTEND_RUTA.'sucursales/'.$_SERVER['HTTP_HOST'].'.txt')) {
    ...
    $values = explode(",",$path);
    if( strstr(dirname(__FILE__), 'C:') ){  
      define('HOST', 'localhost/admincuestionario');  
    }
    else{
      define('HOST', $_SERVER['HTTP_HOST'].'/desarrollov2');  
    }   
    ...
    if (empty($rs)){
      Utils::doRedirect(PUERTO.'://'.$_SERVER['HTTP_HOST'].'/desarrollov2/error/paginanoencontrada.php');
      exit;
    }
    ...
    if( strstr(dirname(__FILE__), 'C:') ){  
      ...
      define('HOST', $rs['dominio'].'/desarrollov2');  
    }
